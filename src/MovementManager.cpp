...
void CMovementManager::move(pos_t x, pos_t y)
{
	...
	time_last_call		 = get_current_time();
	time_for_destination = static_cast<long long>(
								tmpz / static_cast<double>(speed) * 1000.0);// (time_last_call + tmpz * (speed / 1000)) - time_last_call;
...
//this game has the worst programmedclient ive ever seen: fps dependent movement speed on opponents.
// literally the worst thing after tomatoes
CMovementManager::duration_t CMovementManager::get_time_for_destination()
{
	/* DEPRECATED! SWF IS FIXED NOW (thx bugpoint) *\/
	// *0.7 on dx != 0 and dy == 0 or dx == 0 and dy != 0 almost perfect
	// longest distance difference on 45° angles (45°/135°/...) *0.57 was best
	// best way to limit a f(x)=sin(x) is to 2*x is result since it normally jumps back to y=0 on 180*x
	if (g_DBG_useNewSpeedCalc)
	{
		constexpr double min = 0.57;
		constexpr double max = 0.70;
		constexpr double diff = (max - min);
		//tmpz = h
		//xlocal|ygoal
		double deg = std::acos((double)(local_pos.first - planned_pos.first) / tmpz);
		double res = std::abs(diff *std::sin(2 * deg));
		//std::cout << "On " << deg << "° value: " << res << std::endl;
		return time_for_destination * (max - res);
	}
	else
	{*/
...
CMovementManager::duration_t CMovementManager::recalc_time_for_destination()
{
	local_pos = get_current_position();
	...
	time_last_call = get_current_time();
	time_for_destination = static_cast<long long>(
		tmpz / static_cast<double>(speed) * 1000.0);
	return time_for_destination;
}
